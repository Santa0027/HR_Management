...
const RentalCard = ({ rental, onView, onEdit, onDelete, onReturn, onExtend }) => {
  ...
  const isOverdue = () => {
    return new Date(rental.rental_end_date) < new Date() && rental.rental_status === 'ACTIVE';
  };
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex flex-col items-end space-y-1">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(rental.rental_status)}`}>
            {isOverdue() ? 'OVERDUE' : rental.rental_status}
          </span>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-900 font-medium">{rental.renter_name}</span>
        </div>
        <div className="flex items-center space-x-2">
          <Phone className="w-4 h-4 text-gray-400" />
          <span className="text-sm text-gray-600">{rental.renter_contact}</span>
        </div>
        <div className="flex items-center space-x-2">
          <Key className="w-4 h-4 text-gray-400" />
          <span className="text-sm text-gray-600">License: {rental.renter_license}</span>
        </div>
      </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-gray-900">{new Date(rental.rental_start_date).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">End Date:</span>
          <span className="text-sm font-medium text-gray-900">{new Date(rental.rental_end_date).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">Duration:</span>
          <span className="text-sm font-medium text-gray-900">{rental.total_days} days</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">Daily Rate:</span>
          <span className="text-sm font-medium text-gray-900">${rental.daily_rate}</span>
        </div>
        <div className="flex items-center justify-between">
        ...
        {rental.rental_status === 'ACTIVE' && (
          <div className="flex items-center justify-between">
            ...
            <span className={`text-sm font-medium ${getDaysRemaining() < 0 ? 'text-red-600' : getDaysRemaining() <= 3 ? 'text-yellow-600' : 'text-green-600'}`}>
...
const VehicleRentalManagement = () => {
  ...
  useEffect(() => {
    const mockRentals = [
      {
        ...
        renter_contact: '+1-555-0123',
        renter_license: 'DL123456789',
        renter_address: '123 Main St, City, State',
        rental_start_date: '2024-01-10T09:00:00',
        rental_end_date: '2024-01-20T18:00:00',
      ...
      },
      {
        ...
        renter_contact: '+1-555-0456',
        renter_license: 'DL987654321',
        renter_address: '456 Oak Ave, City, State',
        rental_start_date: '2024-01-05T10:00:00',
        rental_end_date: '2024-01-12T17:00:00',
        actual_return_date: '2024-01-12T16:30:00',
        daily_rate: 40.00,
        total_days: 7,
        base_amount: 280.00,
      ...
      },
      {
        ...
        renter_contact: '+1-555-0789',
        renter_license: 'DL456789123',
        renter_address: '789 Pine St, City, State',
        rental_start_date: '2024-01-01T08:00:00',
        rental_end_date: '2024-01-08T20:00:00',
        actual_return_date: null,
        daily_rate: 65.00,
        total_days: 7,
        base_amount: 455.00,
    ...
    // Calculate stats
    const totalRevenue = mockRentals.reduce((sum, rental) => sum + rental.total_amount, 0);
    const avgDailyRate = mockRentals.reduce((sum, rental) => sum + rental.daily_rate, 0) / mockRentals.length;
    
    const stats = {
      totalRentals: mockRentals.length,
      activeRentals: mockRentals.filter(r => r.rental_status === 'ACTIVE').length,
      completedRentals: mockRentals.filter(r => r.rental_status === 'COMPLETED').length,
      overdueRentals: mockRentals.filter(r => new Date(r.rental_end_date) < new Date() && r.rental_status === 'ACTIVE').length,
      totalRevenue: totalRevenue,
      avgDailyRate: avgDailyRate,
      thisMonth: mockRentals.filter(r => new Date(r.rental_start_date).getMonth() === new Date().getMonth()).length,
      pendingPayments: mockRentals.filter(r => r.payment_status === 'PENDING' || r.payment_status === 'PARTIAL').length,
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(rental =>
        rental.vehicle.vehicle_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.vehicle.vehicle_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.renter_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rental.renter_contact.includes(searchTerm)
      );
    }

    if (statusFilter !== 'all') {
      filtered = filtered.filter(rental => rental.rental_status === statusFilter);
    }
  ...
  }, [rentals, searchTerm, statusFilter, paymentFilter]);

  const handleViewRental = (rental) => {
    navigate(`/vehicle-rental/${rental.id}`);
  };

  const handleEditRental = (rental) => {
    navigate(`/vehicle-rental-edit/${rental.id}`);
  };

  const handleDeleteRental = (rental) => {
    if (window.confirm('Are you sure you want to delete this rental record?')) {
      const updatedRentals = rentals.filter(r => r.id !== rental.id);
      setRentals(updatedRentals);
      toast.success('Rental record deleted successfully');
    }
  };

  const handleReturnVehicle = (rental) => {
    navigate(`/vehicle-rental-return/${rental.id}`);
  };

  const handleExtendRental = (rental) => {
    navigate(`/vehicle-rental-extend/${rental.id}`);
  };
  ...
  return (
    <div className="p-6 max-w-8xl mx-auto font-inter">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Vehicle Rental Management</h1>
          <p className="text-gray-600 mt-2">Manage vehicle rentals, bookings, and returns</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">{rentalStats.thisMonth} this month</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{rentalStats.activeRentals}</p>
              <p className="text-xs text-gray-500">{rentalStats.overdueRentals} overdue</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">Avg: ${rentalStats.avgDailyRate.toFixed(2)}/day</p>
            </div>
            <div className="p-3 rounded-full bg-green-100">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{rentalStats.pendingPayments}</p>
